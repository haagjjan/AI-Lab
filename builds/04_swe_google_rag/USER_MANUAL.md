...
API policy. The local model notes in
`../01_mini_rag/google_models.md` list alternatives used in this AI-Lab.

## 2. Open the project

From the AI-Lab repository root:
...
The copied file already includes the standart working defaults:
(I would suggest to keep the embedding dimensions at 768, with xyz can be played around)
...
Relative data paths are resolved from the `04_swe_google_rag` project
directory, not from the terminal's current directory or a custom env file's
directory.
...
The repository's local working copy already contains the first two chapter
PDFs. In a clean checkout, PDFs may need to be added again because this folder
...
For the two supplied local chapters, the verified output is:
