...
Provider — API key (OpenAI/Anthropic) or local via Ollama on your 24GB Mac. Whichever you can get responding fastest today; don't deliberate.
Data — drop 5–10 small text files in data/ (your own diary entries, some articles, anything). The data doesn't matter, the pipeline does.
Indexing flow — load → split into chunks → embed → store. Use a simple vector lib (Chroma or FAISS) for storage/search, but write the orchestration yourself. No LangChain chains. You want to feel the moving parts, and it keeps your framework decision (still deferred) open.
